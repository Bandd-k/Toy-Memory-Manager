...
int HowManyMem();//функция для подсчета запрашиваемой памяти
void DeinitializeMemoryManager();//функция освобождение динамически выделяемой памяти
void HowManyBytes(string a,int &typebyte);//функция для подсчета памяти занимаемой типом
void myadress(int y);//функция для вывода внутреннего адреса
void Mycin(string &type);//функция для считывания строки с пробелом
...
size_t MemSize;//переменная для хранения запрашиваемой памяти
int howmanyblocks=0;
int size;
struct BlockInf{
    void* adress;
...
int main()
{
    ...
    while(!(menu==6)){
        ...
        if(menu==1)
            ToyMp_AllocateBlock(HowManyMem());
        if(menu==2){
            coutBlocks();
            ...
            if((NumberBlock<Blocks.size())&&(NumberBlock>=0))//проверка ввода!на существование блока
                toymp_FreeBlock(Blocks[NumberBlock].adress);
            else
...
void* ToyMp_AllocateBlock(size_t SizeBlock){//функция для выделения блока памяти
    ...
    if(SizeBlock%8==0)//округление в большую сторону
        ...
        cell=SizeBlock/8+1;//
    stack=HowManyemptyslots();//присвоение свободного участка самого большого размера
    for(i=0;i<MemSize/8;i++){
        if(Mymem[i]==0)
            j++;
        else{
    ...
    BlockInf dummy;
    if(SizeBlock/8.<=stack){//достаточно ли памяти?
        for(i=start;i<cell+start;i++)//если да то заполняются свободные ячейки
            Mymem[i]=1;
        cout<<"Машинный адрес выделенного участка:"<<Mymem+start<<"\n";
        cout<<"Внутренний адрес выделенного участка:";
        myadress(start);
        Blocks.push_back(dummy);
        Blocks[howmanyblocks].start=start;
        Blocks[howmanyblocks].adress=Mymem+start;
        Blocks[howmanyblocks].sizecell=cell;
        Blocks[howmanyblocks].blockmem=SizeBlock;
        howmanyblocks++;
        return (Mymem+start);
    }
    else{
...
int HowManyMem(){//функция для подсчета памяти
    ...
    bool cinStruct=1;
    char mas[10];//массив для считывание остатков потока
    int typebyte=0,quantity=0;
    size_t bytes=0;
    while (bytes==0) {//цикл для корректности ввода спецификации данных
        ...
        if(Specific=="struct"){
            while(cinStruct){
                cout<<"Введите параметр(char,short int,double,int,long int,float,end-для конца):";
                Mycin(type);
                if(type=="end"){
                    cinStruct=0;
                    break;
                ...
                HowManyBytes(type,typebyte);
                if(typebyte==0)
                    cout<<"Вы неверно ввели тип!\n";
                bytes+=typebyte;//суммирование памяти занимаемой типами
        ...
        if(Specific=="array"){
            while (cinStruct) {//цикл проверки корректности ввода
            ...
            bytes=quantity*typebyte;//умножение кол-ва элементов на тип
...
void HowManyBytes(string a,int &typebyte){//для подсчета памяти занимаемой типом
    typebyte=0;
    if(a=="char")
        typebyte=sizeof(char);
    if(a=="short int")
        typebyte=sizeof(short int);
    if(a=="double")
        typebyte=sizeof(double);
    if(a=="int")
        typebyte=sizeof(int);
    if(a=="long int")
        typebyte=sizeof(long int);
    if(a=="float")
...
void Mycin(string &type){//функция для считывание строки с пробелом
    char mass[10];
    cin.getline(mass,10);//считывание строки char
    type=mass;//преобразование char в string
...
void coutBlocks(){//вывод информации о блоке
    int i;
    for(i=0;i<howmanyblocks;i++){
        ...
        cout<<"Внутренний адрес:";
        myadress(Blocks[i].start);
    }
}
...
void* realloc ( void * ptr, size_t size ){
    int i=0,newMem;
    for(i=0;i<howmanyblocks;i++){//поиск блока с таким же адресом
        if(Blocks[i].adress==ptr)
            break;
    }
    newMem=Blocks[i].blockmem+size;
    toymp_FreeBlock(ptr);//удаляет старый блок
    if(ToyMp_AllocateBlock(newMem)){ //ищет блок с новой памятью
        return Blocks[howmanyblocks].adress;
    }
    else{
        ToyMp_AllocateBlock(newMem-size);//выделяет память для старого блока
